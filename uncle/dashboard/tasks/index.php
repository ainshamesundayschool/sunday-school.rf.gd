...
<body>
    ...
    <header class="tasks-subpage-header"
      ...
      <div class="header-stats-actions-wrap"
        ...
        <div class="header-actions-box" style="display: flex; gap: 8px; align-items: center;">
          ...
          <button class="btn-create" onclick="openBulkTasksModal()"
            style="background:linear-gradient(135deg, #10b981 0%, #059669 100%); color:#fff; border:none; padding: 0 14px; font-size: 0.85rem; font-weight: 800; height: 38px; display: inline-flex; align-items: center; gap: 6px; white-space: nowrap; flex-shrink: 0; border-radius: var(--r-md); cursor:pointer; box-shadow:0 4px 14px rgba(16,185,129,0.3);"><i
              class="fas fa-file-excel"></i> إضافة مجموعة تاسكات</button>
  ...
  <!-- BULK TASKS CREATION MODAL -->
  <div id="bulkTasksModal" class="taranim-onboard-overlay" style="display:none;">
    <div class="taranim-onboard-card" style="max-width:880px; width:100%; max-height:88vh; max-height:88dvh; display:flex; flex-direction:column; padding:24px !important; text-align:right !important; border-radius:24px;">
      <!-- Modal Header -->
      <div style="display:flex; align-items:center; justify-content:space-between; padding-bottom:14px; border-bottom:1px solid var(--bdr, #e2e8f0);">
        <h3 style="font-family:'Baloo Bhaijaan 2', sans-serif; font-size:1.2rem; font-weight:700; color:var(--t1); margin:0; display:flex; align-items:center; gap:8px;">
          <i class="fas fa-file-excel" style="color:#10b981;"></i> إضافة مجموعة تاسكات دفعة واحدة (Bulk Tasks Add)
        </h3>
        <button type="button" onclick="closeBulkTasksModal()" style="background:var(--bg2, #f1f5f9); border:none; width:32px; height:32px; border-radius:50%; color:var(--t2); cursor:pointer; display:flex; align-items:center; justify-content:center;">
          <i class="fas fa-times"></i>
        </button>
      </div>
      ...
      <div style="overflow-y:auto; flex:1; padding:16px 4px; display:flex; flex-direction:column; gap:16px; text-align:right;">
        ...
        <!-- Action Header: Download Template & Instructions -->
        <div style="background:var(--bg2, #f8fafc); border:1px dashed var(--bdr, #cbd5e1); border-radius:16px; padding:16px; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:12px;">
          <div>
            <div style="font-weight:700; font-size:0.95rem; color:var(--t1); margin-bottom:4px;">
              <i class="fas fa-info-circle" style="color:var(--brand, #6366f1); margin-left:4px;"></i> تعليمات إضافة التاسكات
            </div>
            <div style="font-size:0.8rem; color:var(--t2); line-height:1.6;">
              حمل نموذج شيت التاسكات الفارغ لتعبئة بيانات التاسكات، ثم ارفعه أو انسخ محتواه مباشرة.
            </div>
          </div>
          <button type="button" onclick="downloadEmptyTasksSheetTemplate()" class="btn" style="background:linear-gradient(135deg, #10b981 0%, #059669 100%); color:#fff; border:none; border-radius:12px; padding:8px 16px; font-weight:700; font-size:0.82rem; cursor:pointer; display:inline-flex; align-items:center; gap:8px; box-shadow:0 4px 12px rgba(16,185,129,0.25);">
            <i class="fas fa-download"></i> تحميل نموذج شيت التاسكات (CSV)
          </button>
        </div>
        ...
        <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap:14px;">
          <!-- File Drop Zone -->
          <div style="border:2px dashed var(--bdr); border-radius:16px; padding:16px; text-align:center; background:var(--bg2); transition:all 0.2s ease; cursor:pointer;" onclick="document.getElementById('bulkTasksFileInput').click()">
            <i class="fas fa-cloud-upload-alt" style="font-size:2rem; color:var(--brand); margin-bottom:8px; display:block;"></i>
            <div style="font-weight:700; font-size:0.88rem; color:var(--t1); margin-bottom:4px;">رفع ملف شيت تاسكات (Excel / CSV)</div>
            <div style="font-size:0.75rem; color:var(--t3);">اضغط هنا لاختيار ملف .csv أو .xlsx أو .xls</div>
            <input type="file" id="bulkTasksFileInput" accept=".csv, .xlsx, .xls, .txt" style="display:none;" onchange="handleBulkTasksFileUpload(event)" />
            <div id="bulkTasksFileName" style="margin-top:8px; font-size:0.8rem; font-weight:700; color:#10b981;"></div>
          </div>
          ...
          <div style="display:flex; flex-direction:column; gap:6px;">
            ...
            <textarea id="bulkTasksPasteArea" placeholder="قم بنسخ جدول التاسكات من Excel ولصقه هنا مباشرة..." oninput="parseBulkTasksPasteText()" style="flex:1; min-height:90px; width:100%; border-radius:12px; border:1px solid var(--bdr); background:var(--bg2); color:var(--t1); padding:10px; font-size:0.8rem; font-family:monospace; resize:vertical; outline:none;"></textarea>
          </div>
        </div>
        ...
        <div>
          <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:8px;">
            <div style="font-weight:700; font-size:0.9rem; color:var(--t1); display:flex; align-items:center; gap:6px;">
              <i class="fas fa-list-check" style="color:var(--brand);"></i> معاينة التاسكات المستخرجة
            </div>
            <div id="bulkTasksCountBadge" style="font-size:0.78rem; font-weight:700; color:var(--t3); background:var(--bg2); padding:2px 10px; border-radius:20px; border:1px solid var(--bdr);">
              0 تاسكات
            </div>
          </div>
          ...
          <div style="max-height:220px; overflow-y:auto; border:1px solid var(--bdr); border-radius:14px; background:var(--bg);">
            <table style="width:100%; border-collapse:collapse; text-align:right; font-size:0.8rem;">
              <thead style="background:var(--bg2); position:sticky; top:0; font-weight:700; color:var(--t1); border-bottom:1px solid var(--bdr);">
                <tr>
                  <th style="padding:8px 10px; width:35px; text-align:center;">#</th>
                  <th style="padding:8px 10px;">عنوان التاسك</th>
                  <th style="padding:8px 10px;">الوصف</th>
                  <th style="padding:8px 10px;">الفصل</th>
                  <th style="padding:8px 10px; width:70px; text-align:center;">الدرجة</th>
                  <th style="padding:8px 10px; width:80px; text-align:center;">التصنيف</th>
                  <th style="padding:8px 10px; width:70px; text-align:center;">الحالة</th>
                  <th style="padding:8px 10px; width:70px; text-align:center;">الصلاحية</th>
                </tr>
              </thead>
              <tbody id="bulkTasksPreviewTbody">
                <tr>
                  <td colspan="8" style="text-align:center; padding:24px; color:var(--t3);">
                    قم برفع ملف أو لصق بيانات المعاينة لعرض التاسكات هنا
                  </td>
                </tr>
              </tbody>
            </table>
      ...
      <!-- Modal Footer -->
      <div style="display:flex; align-items:center; justify-content:flex-end; gap:10px; padding-top:14px; border-top:1px solid var(--bdr, #e2e8f0);">
        <button type="button" onclick="closeBulkTasksModal()" style="padding:10px 18px; border-radius:12px; border:1px solid var(--bdr); background:var(--bg2); color:var(--t2); font-weight:700; font-size:0.85rem; cursor:pointer;">
          إلغاء
        </button>
        <button type="button" id="btnConfirmBulkTasksImport" onclick="confirmImportBulkTasks()" disabled style="padding:10px 22px; border-radius:12px; border:none; background:linear-gradient(135deg, #10b981 0%, #059669 100%); color:#fff; font-weight:700; font-size:0.85rem; cursor:pointer; opacity:0.5; display:inline-flex; align-items:center; gap:8px; box-shadow:0 4px 14px rgba(16,185,129,0.3);">
          <i class="fas fa-plus-circle"></i> إنشاء التاسكات المقبولة
        </button>
      </div>
  ...
  <script>
    // ══════════════════════════════════════════════════════════════════
    // BULK TASKS IMPORT & CREATION
    // ══════════════════════════════════════════════════════════════════
    window.parsedBulkTasksList = [];

    function downloadEmptyTasksSheetTemplate() {
      var csvContent = "\uFEFF"; // UTF-8 BOM for Excel Arabic support
      csvContent += "عنوان التاسك,الوصف,الفصل,تاريخ البداية,الموعد النهائي,إجمالي الدرجات,عدد الكوبونات,التصنيف,الحالة\n";
      csvContent += "تاسك الطقس 1 - طقس الكنيسة,حل أسئلة كتاب الطقس صفحة 12,كل الفصول,2026-08-15 00:00,2026-08-22 23:59,100,10,طقس,منشور\n";
      csvContent += "تاسك العقيدة 2 - قانون الإيمان,دراسة وفهم أسئلة قانون الإيمان,ابتدائي,2026-08-15 00:00,بدون موعد,50,5,عقيدة,منشور\n";
      csvContent += "تاسك حفظ آية - يوحنا 3: 16,حفظ وتسميع الآية المقررة,كل الفصول,2026-08-15 00:00,2026-08-30 23:59,30,3,حفظ,مسودة\n";

      var blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
      var link = document.createElement("a");
      var url = URL.createObjectURL(blob);
      link.setAttribute("href", url);
      link.setAttribute("download", "نموذج_تاسكات_جديدة.csv");
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      if (typeof showToast === 'function') showToast('تم تحميل نموذج شيت التاسكات بنجاح!', 'ok');
    }

    function openBulkTasksModal() {
      var modal = document.getElementById('bulkTasksModal');
      if (modal) {
        modal.style.display = 'flex';
        window.parsedBulkTasksList = [];
        var fileInp = document.getElementById('bulkTasksFileInput');
        var pasteArea = document.getElementById('bulkTasksPasteArea');
        var fileName = document.getElementById('bulkTasksFileName');
        if (fileInp) fileInp.value = '';
        if (pasteArea) pasteArea.value = '';
        if (fileName) fileName.textContent = '';
        renderBulkTasksPreview();
      }
    }
    ...
    function handleBulkTasksFileUpload(event) {
      var file = event.target.files[0];
      if (!file) return;

      var nameEl = document.getElementById('bulkTasksFileName');
      if (nameEl) nameEl.textContent = '📄 ' + file.name;
      ...
      if (ext === 'xlsx' || ext === 'xls') {
        ...
        reader.onload = function(e) {
          ...
          processRowsToTasks(rows);
        };
        reader.readAsText(file);
      }
    }

    function parseBulkTasksPasteText() {
      var pasteArea = document.getElementById('bulkTasksPasteArea');
      if (!pasteArea) return;
      var text = pasteArea.value.trim();
      if (!text) {
    ...
    function processRowsToTasks(rows) {
      window.parsedBulkTasksList = [];
      if (!rows || rows.length === 0) {
        renderBulkTasksPreview();
        return;
      ...
      var startIndex = 0;
      var firstRowStr = (rows[0] || []).join(' ').toLowerCase();
      if (firstRowStr.indexOf('عنوان') !== -1 || firstRowStr.indexOf('تاسك') !== -1 || firstRowStr.indexOf('title') !== -1) {
        startIndex = 1;
      }

      var nowStr = new Date().toISOString().slice(0, 10) + ' 00:00:00';
      ...
      for (var i = startIndex; i < rows.length; i++) {
        ...
        var className = (row[2] || '').toString().trim();
        var startDateRaw = (row[3] || '').toString().trim();
        var endDateRaw = (row[4] || '').toString().trim();
        var deg = parseInt(row[5]) || 100;
        var coupons = parseInt(row[6]) || 10;
        var groupName = (row[7] || '').toString().trim();
        var rawStatus = (row[8] || '').toString().trim().toLowerCase();
        ...
        var startDate = startDateRaw ? startDateRaw : nowStr;
        var noDeadline = 0;
        var endDate = endDateRaw;

        if (!endDateRaw || endDateRaw.indexOf('بدون') !== -1 || endDateRaw.indexOf('no') !== -1) {
          noDeadline = 1;
          endDate = '9999-12-31 23:59:59';
        }

        var status = (rawStatus.indexOf('مسودة') !== -1 || rawStatus.indexOf('draft') !== -1) ? 'draft' : 'published';

        window.parsedBulkTasksList.push({
          title: title,
          description: desc,
    ...
    function renderBulkTasksPreview() {
      var tbody = document.getElementById('bulkTasksPreviewTbody');
      var badge = document.getElementById('bulkTasksCountBadge');
      var btn = document.getElementById('btnConfirmBulkTasksImport');
      if (!tbody) return;

      var list = window.parsedBulkTasksList || [];
      var validCount = list.filter(function(t) { return t.valid; }).length;

      if (badge) badge.textContent = validCount + ' تاسكات صالحة من أصل ' + list.length;

      if (list.length === 0) {
        tbody.innerHTML = '<tr><td colspan="8" style="text-align:center; padding:24px; color:var(--t3);">قم برفع ملف أو لصق بيانات المعاينة لعرض التاسكات هنا</td></tr>';
        if (btn) { btn.disabled = true; btn.style.opacity = '0.5'; }
        return;
      }
      ...
      list.forEach(function(t, i) {
        var statusBadge = t.status === 'published'
          ? '<span style="background:#dcfce7; color:#15803d; padding:2px 8px; border-radius:6px; font-weight:700; font-size:0.75rem;">منشور</span>'
          : '<span style="background:#fef3c7; color:#b45309; padding:2px 8px; border-radius:6px; font-weight:700; font-size:0.75rem;">مسودة</span>';

        var validityBadge = t.valid
          ? '<span style="color:#10b981; font-weight:700;"><i class="fas fa-check-circle"></i> صالح</span>'
          : '<span style="color:#ef4444; font-weight:700;" title="' + esc(t.errorMsg) + '"><i class="fas fa-exclamation-circle"></i> خطأ</span>';

        html += '<tr style="border-bottom:1px solid var(--bdr); ' + (!t.valid ? 'background:rgba(239,68,68,0.05);' : '') + '">' +
          '<td style="padding:8px 10px; text-align:center; font-weight:700;">' + (i + 1) + '</td>' +
          '<td style="padding:8px 10px; font-weight:700; color:var(--t1);">' + esc(t.title) + '</td>' +
          '<td style="padding:8px 10px; color:var(--t2); font-size:0.75rem;">' + esc(t.description || '—') + '</td>' +
          '<td style="padding:8px 10px; color:var(--t2); font-size:0.75rem;">' + esc(t.class_name || 'كل الفصول') + '</td>' +
          '<td style="padding:8px 10px; text-align:center; font-weight:700; color:var(--brand);">' + t.total_degree + '</td>' +
          '<td style="padding:8px 10px; text-align:center; font-size:0.75rem; font-weight:600;">' + esc(t.group_name || '—') + '</td>' +
          '<td style="padding:8px 10px; text-align:center;">' + statusBadge + '</td>' +
          '<td style="padding:8px 10px; text-align:center;">' + validityBadge + '</td>' +
          '</tr>';
      });
    ...
    async function confirmImportBulkTasks() {
      var validTasks = (window.parsedBulkTasksList || []).filter(function(t) { return t.valid; });
      if (validTasks.length === 0) {
        if (typeof showToast === 'function') showToast('لا توجد تاسكات صالحة للإضافة', 'err');
        return;
      }

      var btn = document.getElementById('btnConfirmBulkTasksImport');
      if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> جاري إضافة التاسكات...'; }

      var successCount = 0;
      for (var i = 0; i < validTasks.length; i++) {
        var t = validTasks[i];
        if (typeof showToast === 'function') showToast('جاري إنشاء التاسك (' + (i + 1) + ' من ' + validTasks.length + ')...', 'info');

        try {
          var res = await api('createTask', {
            title: t.title,
            description: t.description,
            class_name: t.class_name,
            start_date: t.start_date,
            end_date: t.end_date,
            no_deadline: t.no_deadline ? 1 : 0,
            total_degree: t.total_degree,
            max_coupons: t.max_coupons,
          ...
          if (res && res.success) {
            ...
            console.warn('Failed to create task:', t.title, res);
          }
        } catch (e) {
          console.error('Error creating bulk task:', e);
        }
      }

      if (typeof showToast === 'function') showToast('تم إنشاء ' + successCount + ' تاسك بنجاح!', 'ok');
      closeBulkTasksModal();
